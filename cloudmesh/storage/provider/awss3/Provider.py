...
class Provider(object):
    ...
    def joinFileNameDir(self, fileName, dirName):
        ...
        if (len(self.trimFileNamePath(dirName)) > 0):
            fullFilePath = self.trimFileNamePath(
                dirName) + '/' + self.trimFileNamePath(fileName)
        else:
    ...
    def create_dir(self, service=None, directory=None):
        ...
        """
        fileContent = ""
        filePath = self.joinFileNameDir(self.directory_marker_file_name,
                                        directory)
        ...
        obj = list(self.s3Resource.Bucket(self.container_name).objects.filter(
            Prefix=filePath))

        if len(obj) == 0:
            markerObject = self.s3Resource.Object(self.container_name,
                                                  filePath).put(
                Body=fileContent)
            ...
            for obj in objs:
                if obj.key.startswith(self.trimFileNamePath(trimmedSource)):
                    ...
                    if (fileName[-1] == '/'):
                        ...
                        # Its a file
                        if (fileName.replace(trimmedSource, '')[
    ...
    def delete(self, service=None, source=None, recursive=False):
        ...
        try:
            fileObj = self.s3Client.get_object(Bucket=self.container_name,
                                               Key=trimmedSource)
        ...
        if fileObj:
            ...
            allObjs = list(
                self.s3Resource.Bucket(self.container_name).objects.filter(
            ...
            if total_allObjs == 0:
                ...
                for obj in allObjs:
                    # if obj.key.startswith(self.trimFileNamePath(trimmedSource)):
                    self.s3Resource.Object(self.container_name,
                                           obj.key).delete()
    ...
    def put(self, service=None, source=None, destination=None, recursive=False):
        ...
        if isSourceFile is True:
            ...
            self.s3Client.upload_file(trimmedSource, self.container_name,
                                      trimmedDestination)
                ...
                for (dirpath, dirnames, filenames) in os.walk(trimmedSource):
                    for fileName in filenames:
                        ...
                        filesToUpload.append(
                            self.trimFileNamePath(dirpath) + '/' + fileName)

                for file in filesToUpload:
                    self.s3Client.upload_file(file,
                                              self.container_name,
                                              trimmedDestination + '/' + self.trimFileNamePath(
            ...
            if total_allObjs == 0:
                ...
                for obj in allObjs:
                    if os.path.basename(
                        ...
                        if self.trimFileNamePath(
                                obj.key.replace(trimmedSource, '')).count(
                                '/') == 0:
                            try:
                                blob = self.s3Resource.Bucket(
                                    self.container_name).download_file(
                ...
                for obj in allObjs:
                    # print(obj.key)
                    if os.path.basename(
                            obj.key) != self.directory_marker_file_name and \
                            obj.key[-1] != '/':
                        if self.trimFileNamePath(
                                obj.key.replace(trimmedSource, '')).count(
                                '/') == 0:
    ...
    def search(self, service=None, directory=None, filename=None,
        ...
        if (len(directory) > 0) and recursive is False:
            ...
            objs = list(
                self.s3Resource.Bucket(self.container_name).objects.filter(
                    Prefix=self.trimFileNamePath(directory)))
        elif (len(directory) == 0) and recursive is True:
            objs = list(
                self.s3Resource.Bucket(self.container_name).objects.all())

        if len(objs) > 0:
            for obj in objs:
                if self.splitToList(obj.key)[-1] == filename:
                    # print(obj.key)
                    metadata = self.s3Client.head_object(
