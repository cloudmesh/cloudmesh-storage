###############################################################
# pytest -v --capture=no tests/test_storage_azure-old.py
...
class TestStorage(object):
    ...
    def test_put(self):
        ...
        src = "~/.cloudmesh/storage/test/a/a.txt"
        dst = '/'
        Benchmark.Start()
   ...
   # def test_put_recursive(self): # marker this as failure as azure put directory is not working
    ...
    # So the below function only works only for files

    def test_delete(self):
        HEADING()
        src = '/created_dir/dummy.txt'
        Benchmark.Start()
        provider.delete(src)
        provider.run()
        Benchmark.Stop()
